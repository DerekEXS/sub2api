export default {
  agent: {
    title: 'Agent Instances',
    description: 'Manage user PicoClaw container instances: capacity pool, lifecycle status, archive download and per-user config.',
    poolFree: 'Pool Free Space',
    poolActive: 'Active Instances',
    poolQueued: 'Queued',
    poolArchived: 'Archived',
    instanceList: 'Instance List',
    refresh: 'Refresh',
    loading: 'Loading...',
    noInstances: 'No agent instances',
    colUser: 'User ID',
    colIdentity: 'Email / Username',
    colStatus: 'Status',
    statusActive: 'Running',
    statusQueued: 'Queued',
    statusProvisioning: 'Provisioning',
    statusRetained: 'Archived',
    statusOverQuota: 'Quota Paused',
    statusError: 'Error',
    colHost: 'Access Host',
    colHardcap: 'Hard Cap',
    colActions: 'Actions',
    download: 'Archive',
    destroy: 'Destroy',
    destroyConfirm: 'Destroy instance of user',
    destroyed: 'Destroyed & archived (user {id}): container removed, data in retention, restart within retention restores it',
    destroyFailed: 'Destroy failed: ',
    globalConfig: 'Global Config',
    retainHours: 'Retention Period (hours)',
    hardcapHours: 'Hard Cap (hours)',
    quotaMB: 'Workspace Quota (MB)',
    memoryMB: 'Memory Quota (MB)',
    idleMinutes: 'Idle Timeout (minutes)',
    save: 'Save',
    saved: 'Saved',
    userConfig: 'Config',
    clearOverride: 'Clear Overrides',
    effectiveHint: 'Effective: retention {retention}h · hardcap {hardcap}h · quota {quota}MB · memory {memory}MB · idle {idle}min',
    hwTitle: 'Agent Backend Hardware',
    hwUptime: 'Uptime',
    hwContainers: 'Containers',
    hwCores: 'cores',
    hwMemory: 'Memory',
    hwDisk: 'Disk',
    hwUnavailable: 'Hardware metrics unavailable (manager outdated or unreachable)',
  },
}
