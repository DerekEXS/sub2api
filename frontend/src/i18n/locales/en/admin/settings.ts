export default {
    settings: {
      ...
      registrationAudit: {
        ...
        colIp: 'IP',
        colTime: 'Created At',
        gotoUser: 'Open user management (search by email)',
        pagePrev: 'Previous',
        pageNext: 'Next',
        pageInfo: 'Page {page} · {total} total',
        configTitle: 'Risk Rule Configuration',
        configDescription: 'Tune thresholds and scores for the registration risk assessment. Save applies to new registrations immediately.',
        configHowItWorks: 'How it works: each new registration accumulates a risk score from the rules below (a hit adds its score; negative = bonus). Total ≥ Flag Threshold puts the user on this list; total ≥ Strong Threshold marks it ⚠️ strong (review first). Scoring only flags — it never rejects registrations (except UA Check).',
        configUaCheck: 'UA Check',
        configUaCheckHint: 'Hard-reject automated clients (Go-http-client, python-requests etc.). This is the only hard block; all other rules only score.',
        configCgnatExempt: 'CGNAT Exemption',
        configCgnatExemptHint: 'Skip IP-frequency scoring for shared carrier CGNAT egress IPs (100.64.0.0/10) to avoid punishing users behind the same campus/ISP NAT.',
        configFlagThreshold: 'Flag Threshold',
        configFlagThresholdHint: 'Total score at or above this lands on the audit list (default 40)',
        ...
        configScoreRhythmHint: 'Added when the same IP registers ≥2 accounts within 10 minutes (default +20)',
        configScore24h45: '24h Mid-freq +pts',
        configScore24h45Hint: 'Added for the 4th–5th registration from one IP within 24h (default +30)',
        configScore24h6plus: '24h High-freq +pts',
        configScore24h6plusHint: 'Added from the 6th registration within 24h (default +60, almost certainly bulk accounts)',
        configScoresHint: 'Example: random email (+20) + alias (+20) + 4th sign-up in 24h (+30) = 70 → above strong threshold 60, marked ⚠️. Whitelist (-10) offsets some risk.',
        configSaved: 'Risk rule configuration saved',
        configSaveFailed: 'Failed to save risk rule configuration',
      },
      panelRateLimit: {
      ...
      payment: {
        ...
        settlementCurrencyHint: 'Currency of the balance after recharge (default USD: get USD balance)',
        rechargeCurrency: 'Recharge Currency',
        rechargeCurrencyHint: 'Default input currency on the payment page (default CNY)',
        currencyUsd: 'US Dollar (USD)',
        currencyCny: 'Chinese Yuan (CNY)',
        currencyEur: 'Euro (EUR)',
        fxApiUrl: 'FX API URL',
        fxApiUrlHint: 'JSON API returning a "rates" field (e.g. https://api.example.com/v1/latest/USD). Empty = use fixed rate below',
        fxApiUrlPlaceholder: 'Empty = use fixed rate',
        fxApiUrls: 'FX API URLs (multiple)',
        fxApiUrlsHint: 'One URL per line; fallback chain on failure. A standard FX API already covers USD/CNY/EUR and more.',
        fxApiTest: 'Fetch FX Now',
        fxApiTesting: 'Fetching...',
        fxApiTestSuccess: '✓ {base}/{quote} = {rate} (took {ms} ms)',
        fxApiTestFailed: '✗ {error}',
        fxFallbackRate: 'Fixed FX Rate (CNY per USD)',
        fxFallbackRateHint: 'Fallback when API fails or empty. Auto-applied when settlement/recharge currencies differ',
        currencySeparationPreview: 'Preview: Recharge {recharge} {rc} → Gateway pays {pay} {pc} → Credited {credit} {sc}',
