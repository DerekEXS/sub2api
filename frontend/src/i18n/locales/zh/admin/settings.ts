export default {
    settings: {
      ...
      registrationAudit: {
        ...
        colIp: 'IP',
        colTime: '创建时间',
        gotoUser: '跳转到用户管理（按邮箱搜索）',
        pagePrev: '上一页',
        pageNext: '下一页',
        pageInfo: '第 {page} 页 · 共 {total} 条',
        configTitle: '风险规则配置',
        configDescription: '调整注册风险评估的阈值与分值，保存后对新注册立即生效。',
        configHowItWorks: '工作原理：每个新注册按下方各项规则累计「风险分」（命中一项加对应分值，负分=减分）。总分 ≥「打标阈值」进入本审计名单；总分 ≥「强标阈值」标记为强信号 ⚠️（更可疑，建议优先核查）。打分只做标记，不会自动拒绝注册（UA 检查除外）。',
        configUaCheck: 'UA 检查',
        configUaCheckHint: '直接拒绝自动化工具的注册请求（如 Go-http-client、python-requests 等程序化 UA）。这是唯一的硬拦截项，其余规则只打分不拦截。',
        configCgnatExempt: 'CGNAT 豁免',
        configCgnatExemptHint: '共享出口 IP（运营商 CGNAT，100.64.x.x 段）上的多人注册不计「同 IP 频率」分，避免误伤同一小区/校园的正常用户。',
        configFlagThreshold: '打标阈值',
        configFlagThresholdHint: '风险总分达到此值进入审计名单（默认 40）',
        configStrongThreshold: '强标阈值',
        configStrongThresholdHint: '风险总分达到此值标记 ⚠️ 强信号（默认 60）',
        configEmailLongLocal: '随机邮箱判定长度',
        configEmailLongLocalHint: "邮箱 {'@'} 前部分 ≥ 此长度视为疑似随机串（默认 14，如 xk2j9fq8w3nz7p1{'@'}）",
        configEmailVowelRatio: '随机邮箱元音比例',
        configEmailVowelRatioHint: "{'@'} 前部分元音占比 < 此值视为疑似随机串（默认 0.15；正常人名/单词元音多，随机串辅音多）",
        ...
        configScoreRhythmHint: '同一 IP 10 分钟内 ≥2 次注册时加的分（默认 +20，机器批量注册特征）',
        configScore24h45: '24h 中频 +分',
        configScore24h45Hint: '同一 IP 24 小时内第 4~5 次注册时加的分（默认 +30）',
        configScore24h6plus: '24h 高频 +分',
        configScore24h6plusHint: '同一 IP 24 小时内第 6 次及以上注册时加的分（默认 +60，几乎必是批量小号）',
        configScoresHint: '示例：随机串邮箱(+20) + 带别名(+20) + 同 IP 24h 第 4 次(+30) = 70 分 → 超过强标阈值 60，标记 ⚠️。白名单邮箱(-10)可抵消部分风险分。',
        configSaved: '风险规则配置已保存',
        configSaveFailed: '保存风险规则配置失败',
      },
      panelRateLimit: {
        title: '面板接口限流',
      ...
      payment: {
        ...
        settlementCurrencyHint: '用户充值后到账的余额单位（默认 USD：充值得美元余额）',
        rechargeCurrency: '充值货币',
        rechargeCurrencyHint: '支付页面默认输入的货币（默认 CNY：输入人民币）',
        currencyUsd: '美元 (USD)',
        currencyCny: '人民币 (CNY)',
        currencyEur: '欧元 (EUR)',
        fxApiUrl: '汇率 API 地址',
        fxApiUrlHint: '返回 JSON 含 rates 字段的汇率接口（例：https://api.example.com/v1/latest/USD）。留空则使用下方固定汇率',
        fxApiUrlPlaceholder: '留空使用固定汇率',
        fxApiUrls: '汇率 API 地址（多条）',
        fxApiUrlsHint: '多行填写多条 URL，主失败自动降级到下一条。一份标准汇率 API 已含 USD/CNY/EUR 等全套币种。',
        fxApiTest: '立即获取汇率',
        fxApiTesting: '正在获取...',
        fxApiTestSuccess: '✓ {base}/{quote} = {rate}（耗时 {ms} ms）',
        fxApiTestFailed: '✗ {error}',
        fxFallbackRate: '固定汇率（CNY per USD）',
        fxFallbackRateHint: 'API 失败或未配时的兜底汇率。如启用结算币/充值币分离，会自动按此汇率换算',
        currencySeparationPreview: '预览：充值 {recharge} {rc} → 实际收款 {pay} {pc} → 到账 {credit} {sc} 余额',
