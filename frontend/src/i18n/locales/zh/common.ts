export default {
  ...
  nav: {
    ...
    promptAudit: '提示词审计',
    auditLogs: '操作日志',
    agentService: '云 Agent',
    agentAdmin: '云 Agent 管理',
  },

  // 云 Agent（云端 PicoClaw Agent 一键部署）
  agentService: {
    title: '云 Agent',
    description:
      '一键部署您的专属云 Agent。启动后将自动生成专属云 Agent API 密钥（倍率最优分组、关闭即自动删除）；实例数据完全隔离；空闲超时自动销毁，销毁前请及时下载归档，销毁后数据概不负责。',
    statusTitle: '服务状态',
    ...
    statusQueued: '排队中',
    statusStopping: '关闭中',
    statusRetained: '已关闭·数据保留中',
    statusError: '异常',
    statusUnknown: '未知状态',
    startButton: '启动 云 Agent',
    stopButton: '关闭 云 Agent',
    restartButton: '重新启动 云 Agent',
    downloadButton: '下载归档',
    retainedTitle: '云 Agent 已关闭，数据保留中',
    retainedHint: '实例已停止，工作区数据在保留期内仍可恢复。保留期或硬顶到期后数据将被清理，请及时下载归档。',
    idleLabel: '空闲超时',
    retainLabel: '保留期',
    hardcapLabel: '硬顶剩余',
    ...
    launchNote: '点击启动将自动生成新的云 Agent 密钥并注入实例',
    passwordLabel: '访问密码',
    copyButton: '复制',
    copied: '已复制',
    emptyTitle: '未启动 云 Agent',
    emptyHint:
      '点击「启动 云 Agent」即可创建并运行您的专属 PicoClaw 容器。启动时将自动生成专属「云 Agent」API 密钥（绑定当前倍率最优分组，关闭后自动删除）；实例数据完全隔离。',
    emptyNote:
      '实例空闲一段时间后自动销毁并进入保留期，保留期内重启可恢复数据；保留期与硬顶（自首次启动起算）任一到期，数据即被清除。数据不会永久保存，到达一定时间就会清理，具体时长以 云 Agent 页为准，请及时下载自己的数据。',
