export default {
  ...
  agentService: {
    ...
    stopButton: '关闭 Agent',
    restartButton: '重新启动 Agent',
    downloadButton: '下载归档',
    retainedTitle: 'Agent 已关闭，数据保留中',
    ...
    emptyHint:
      '点击「启动 Agent」即可创建并运行您的专属 PicoClaw 容器。启动时将自动生成专属「Agent」API 密钥（绑定当前倍率最优分组，关闭后自动删除）；实例数据完全隔离。',
    emptyNote:
      '实例空闲一段时间后自动销毁并进入保留期，保留期内重启可恢复数据；保留期与硬顶（自首次启动起算）任一到期，数据即被清除。数据不会永久保存，到达一定时间就会清理，具体时长以 Agent 服务页为准，请及时下载自己的数据。',
