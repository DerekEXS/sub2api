...
type Group struct {
	...
	// 多窗口高峰倍率配置 [{start,end,multiplier,models}]，非空时优先于旧单窗口字段
	PeakWindows []domain.PeakWindow `json:"peak_windows,omitempty"`
...
func (*Group) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		switch columns[i] {
		case group.FieldPeakWindows, group.FieldVideoModelPrices, group.FieldModelPricing, group.FieldModelRouting, group.FieldSupportedModelScopes, group.FieldMessagesDispatchModelConfig, group.FieldModelsListConfig, group.FieldReasoningEffortMappings:
			values[i] = new([]byte)
	...
	for i := range columns {
		...
		case group.FieldPeakWindows:
			if value, ok := values[i].(*[]byte); !ok {
				return fmt.Errorf("unexpected type %T for field peak_windows", values[i])
			} else if value != nil && len(*value) > 0 {
				if err := json.Unmarshal(*value, &_m.PeakWindows); err != nil {
					return fmt.Errorf("unmarshal field peak_windows: %w", err)
				}
			}
...
func (_m *Group) String() string {
	...
	builder.WriteString(", ")
	builder.WriteString("peak_windows=")
	builder.WriteString(fmt.Sprintf("%v", _m.PeakWindows))
	builder.WriteString(", ")
